...
modelo1 <- glm(Positivo ~ `GRUPO MÁSCARA`,family = "binomial", 
               data=banco3)
summary(modelo1)

#modelo 2
bancomod2 <- banco3 %>% dplyr::select(Positivo,2,11,32,34,36,Sexo,Idade)
bancomod2$Positivo <- as.factor(bancomod2$Positivo)
bancomod2$Idade <- as.numeric(bancomod2$Idade)
...
bancomod3$Positivo <- as.factor(bancomod3$Positivo)
bancomod3$Idade <- as.numeric(bancomod3$Idade)
...
xtable::xtable(summary(modeloStep))

#modelo 4
bancomod4 <- banco3 %>% dplyr::select(Positivo,2,11,32) %>% na.omit()
str(bancomod4)
modelo4 <- glm(factor(Positivo) ~. ,family = "binomial", data=bancomod4)
xtable::xtable(summary(modelo4))
...
pacman::p_load("readxl", "dplyr", "purrr",
               ...
               "survey", "MASS", "caret")
modelo2$model$Positivo
#modelo 2
roc(modelo2$model$Positivo, predict(modelo2, type = "response"),
    percent=TRUE,
    col = "red",
    print.auc = TRUE,
    plot=T,
...
conf_mat <- confusionMatrix(factor(as.numeric(predict(modelo2, type = "response")>=0.1)),
                            factor(modelo2$model$Positivo), positive = "1")
xtable::xtable(conf_mat$table)
conf_mat$byClass[c("Sensitivity", "Specificity")]


conf_mat <- confusionMatrix(factor(as.numeric(predict(modelo2, type = "response")>=0.5)),
                            factor(modelo2$model$Positivo), positive = "1")
conf_mat$table
conf_mat$byClass[c("Sensitivity", "Specificity")]
