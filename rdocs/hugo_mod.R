...
df1 <- data_j %>% filter(`GRUPO MÁSCARA`==1)
df2 <- data_j %>% filter(`GRUPO MÁSCARA`==2)

# Análise das variáveis para os testes
table(data_j$`Tomou a vacina contra a gripe (este ano)? (A2)`)
table(df1$`Tomou a vacina contra a gripe (este ano)? (A2)`)
table(df2$`Tomou a vacina contra a gripe (este ano)? (A2)`)
...
table(df1$`Escala de Usabilidade da máscara- A1-Você trocou/emprestou a(s) sua(s) máscara(s) com algum colega em algum momento do estudo?`)
table(df2$`Escala de Usabilidade da máscara- A1-Você trocou/emprestou a(s) sua(s) máscara(s) com algum colega em algum momento do estudo?`)
...
table(df1$`Como você descreveria o grau de calor e suor influenciado pela máscara, no seu rosto, durante o uso? (A1)`)
table(df2$`Como você descreveria o grau de calor e suor influenciado pela máscara, no seu rosto, durante o uso? (A1)`)


table(data_j$`Como você descreveria a facilidade para respirar, durante o uso da máscara? (A1)`)
table(df1$`Como você descreveria a facilidade para respirar, durante o uso da máscara? (A1)`)
table(df2$`Como você descreveria a facilidade para respirar, durante o uso da máscara? (A1)`)


table(data_j$`Qual foi o percentual de uso da máscara durante os plantões?  (Inserir número de 0 a 100)- (A1)`)
table(df1$`Qual foi o percentual de uso da máscara durante os plantões?  (Inserir número de 0 a 100)- (A1)`)
table(df2$`Qual foi o percentual de uso da máscara durante os plantões?  (Inserir número de 0 a 100)- (A1)`)
...
table(df1$`Você trocou/emprestou a(s) sua(s) máscara(s) com algum colega em algum momento do estudo?  (A2)`)
table(df2$`Você trocou/emprestou a(s) sua(s) máscara(s) com algum colega em algum momento do estudo?  (A2)`)


table(data_j$`Como você descreveria o grau de calor e suor influenciado pela máscara, no seu rosto, durante o uso? (A2)`)
table(df1$`Como você descreveria o grau de calor e suor influenciado pela máscara, no seu rosto, durante o uso? (A2)`)
table(df2$`Como você descreveria o grau de calor e suor influenciado pela máscara, no seu rosto, durante o uso? (A2)`)


table(data_j$`Desconforto Musculoesqueletico- Dominância manual`)
table(df1$`Desconforto Musculoesqueletico- Dominância manual`)
table(df2$`Desconforto Musculoesqueletico- Dominância manual`)


table(data_j$`Qual foi o percentual de uso da máscara durante os plantões?  (Inserir número de 0 a 100) - (A2)`)

table(data_j$`Qual foi o percentual de uso da máscara durante os plantões?  (Inserir número de 0 a 100) - (A2)`)
table(df1$`Qual foi o percentual de uso da máscara durante os plantões?  (Inserir número de 0 a 100) - (A2)`)
table(df2$`Qual foi o percentual de uso da máscara durante os plantões?  (Inserir número de 0 a 100) - (A2)`)



table(data_j$`Alguma vez você sentiu desconforto (dor, formigamento, perda de força, etc) nos braços ou pescoço?`)
table(df1$`Alguma vez você sentiu desconforto (dor, formigamento, perda de força, etc) nos braços ou pescoço?`)
table(df2$`Alguma vez você sentiu desconforto (dor, formigamento, perda de força, etc) nos braços ou pescoço?`)


table(data_j$`Você teve desconforto na última semana (últimos sete dias)?  (nos locais indicados na figura abaixo)`)
table(df1$`Você teve desconforto na última semana (últimos sete dias)?  (nos locais indicados na figura abaixo)`)
table(df2$`Você teve desconforto na última semana (últimos sete dias)?  (nos locais indicados na figura abaixo)`)
...
table(df1$`Durante o último mês, quantas horas de sono você teve por noite? (esta pode ser diferente do número de horas que você ficou na cama)`)#retirar 45052
table(df2$`Durante o último mês, quantas horas de sono você teve por noite? (esta pode ser diferente do número de horas que você ficou na cama)`)#retirar 45052


table(data_j$`Como você avalia a qualidade geral do seu sono durante o último mês?`)
table(df1$`Como você avalia a qualidade geral do seu sono durante o último mês?`)
table(df2$`Como você avalia a qualidade geral do seu sono durante o último mês?`)
...
table(df1$`Você tem dificuldade em permanecer acordado enquanto está trabalhando, fazendo refeições, ou envolvido em atividades sociais?`)
table(df2$`Você tem dificuldade em permanecer acordado enquanto está trabalhando, fazendo refeições, ou envolvido em atividades sociais?`)


table(data_j$`Se sim, com qual frequência? (insira o número de vezes por dia)`)
table(df1$`Se sim, com qual frequência? (insira o número de vezes por dia)`)
table(df2$`Se sim, com qual frequência? (insira o número de vezes por dia)`)
...
table(df1$`ICT- Assinale com um número na escala de 0 a 10, quantos pontos você daria para sua capacidade de trabalho atual (no momento presente):`)
table(df2$`ICT- Assinale com um número na escala de 0 a 10, quantos pontos você daria para sua capacidade de trabalho atual (no momento presente):`)

table(data_j$`EUROQOL (Utilidade)`)
table(df1$`EUROQOL (Utilidade)`)
table(df2$`EUROQOL (Utilidade)`)

table(data_j$`JSS- ESCORE ''Demanda psicológica'':`)
table(df1$`JSS- ESCORE ''Demanda psicológica'':`)
table(df2$`JSS- ESCORE ''Demanda psicológica'':`)


table(data_j$`JSS- ESCORE ''Controle no processo de trabalho'':`)
table(df1$`JSS- ESCORE ''Controle no processo de trabalho'':`)
table(df2$`JSS- ESCORE ''Controle no processo de trabalho'':`)


table(data_j$`JSS- ESCORE ''Apoio Social'':`)
table(df1$`JSS- ESCORE ''Apoio Social'':`)
table(df2$`JSS- ESCORE ''Apoio Social'':`)

table(data_j$`JSS- ESCORE FINAL`)
table(df1$`JSS- ESCORE FINAL`)
table(df2$`JSS- ESCORE FINAL`)
